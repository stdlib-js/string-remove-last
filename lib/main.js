...
function removeLast( str ) {
	...
	if ( !isString( str ) ) {
		throw new TypeError( format( '1P93F', str ) );
	}
	opts = {
		'mode': 'grapheme'
	};
	...
	if ( nargs === 1 ) {
		...
		if ( isPlainObject( n ) ) {
			...
			throw new TypeError( format( '1P93X', n ) );
		}
	} else { // nargs > 2
		n = arguments[ 1 ];
		...
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1P92V', options ) );
		}
	}
	if ( options ) {
		if ( hasOwnProp( options, 'mode' ) ) {
			...
			if ( !isMode( opts.mode ) ) {
				throw new TypeError( format( '1P9E1', 'mode', MODES.join( '", "' ), opts.mode ) );
			}
		}
	}
